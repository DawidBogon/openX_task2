...
def find_avalible_slot(calendars, dur_in_mins, min_people):
    def path_leaf(path):
    ...
    # im skipping first element here because its not needed, at least on Windows, im not
    # sure if it will work on linux like this
    # paths to text files
    cal_paths = glob.glob(calendars[1:]+r'\*.txt')
    peoples_names = [path_leaf(path)[:-4] for path in cal_paths]

    # stores busy times for each person:
    busy_dict = {}
    # stores all timestamps, and times 1 second befeor start of being busy and 1 second after not being busy.
    timestamps = []
        ...
        with open(file) as fil:
            for line in fil:
                if len(line) == 11:
                    ...
                    start_date = datetime.strptime(line[:19], '%Y-%m-%d %H:%M:%S')
                    end_date = datetime.strptime(line[22:], '%Y-%m-%d %H:%M:%S')
    ...
    # timestamps.pop(0)
    # DOPYTAĆ SIĘ O TO czy 30 min to 30 in czy 29 minut i 59 sekund
    # im adding extra elemnt, it will be end time of meeting if any earlier time is not suitable
    timestamps.append(timestamps[-1]+dur_in_mins_dt_object)
    # timestamps.insert(0,timestamps[0]-dur_in_mins_dt_object)
    # creating pandas DataFrame with columns dates, and avalibility of each person
    ...
    # setting meeting time in worst case scenario or returning curent time if its later than
    # last elem in array
    ...
    # iterating over every combination of people looking for earliest time to start meeting

    # this is unnecesary, but if lets say we would like to check avbability
...
if __name__ == "__main__":
    ...
    # parser.add_argument("-v",'--verbose',action="store_true",help="print meeting start time and avalible people")
